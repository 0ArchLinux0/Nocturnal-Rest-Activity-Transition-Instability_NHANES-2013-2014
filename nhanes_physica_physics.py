...
def plot_entropy_boxplot(df: pd.DataFrame, savepath: Path):
    """Entropy by sleep problem status (No sleep problem / Sleep problem reported)."""
    ...
    ax.set_title("Shannon Entropy: Circadian Disorder Metric", fontsize=14)
...
def plot_entropy_by_age(df: pd.DataFrame, savepath: Path):
    """Entropy by age stratum (Young / Middle / Older). Uses age percentiles from data."""
    import matplotlib.pyplot as plt
    ...
    fig, ax = plt.subplots(figsize=(6, 5))
    bp = ax.boxplot(data, labels=strata, patch_artist=True)
    for box in bp["boxes"]:
...
def main():
    ...
    plot_entropy_by_age(df, OUTPUT_DIR / "fig5b_entropy_by_age.png")
    print(f"  Saved: {OUTPUT_DIR / 'fig5b_entropy_by_age.png'}")
